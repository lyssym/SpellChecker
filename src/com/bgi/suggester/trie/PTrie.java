...
public class PTrie {
	...
	private Trie trie = SystemConfig.TRIE;
	...
	private void sort(List<Element<String>> result) {
	...
	private List<String> extract(List<Element<String>> result) {
		if (result == null ) return null;
		int size = result.size();
		size = size > 10 ? 10 : size;
		List<String> ret = new ArrayList<String>();
		for (int i = 0; i < size; i++) {
			Element<String> e = result.get(i);
			ret.add(e.data);
		}
		return ret;
	}
    ...
    public void addKey(String word) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            List<Element<String>> ret = trie.searchKeys(key);
 
            sort(ret);
            
            PNode subNode = (PNode)node.values.get(c);
            if (subNode == null) {
                subNode = new PNode();
    ...
    public List<String> queryKey(String word) {
        ...
        for (int i = 0; i < size; i++) {
            char c = word.charAt(i);
            subNode = (PNode)node.values.get(c);
    ...
    public boolean deleteKey(PNode current, String word, int index) {
        ...
        PNode node = (PNode)current.getChildren().get(c);
        if (node == null) {
            return false;
        }
